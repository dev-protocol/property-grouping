...
contract PropertyDirectory is PropertyDirectoryStorage, UsingConfig{
	...
	modifier onlyFactory() {
		require(msg.sender == IPropertyDirectoryConfig(configAddress()).getFactory(), "factory only.");
		_;
	}

	function associate(address _property, uint _amount) external {
		require(propertySet.length() < MAC_ASSOCIATE_COUNT, "over associate count");
		validatePropertyAddress(_property);

		address token = IPropertyDirectoryConfig(configAddress()).getToken(address(this));
		IERC20 directoryToken = IERC20(token);
		uint256 transferdPropertyBalance = getTransferedProperty(_property, msg.sender);
		uint256 directoryTokenBalance = directoryToken.balanceOf(msg.sender);
		uint256 mulAmount = _property.mulAmount(_amount);
		require(transferdPropertyBalance + mulAmount < directoryTokenBalance, "directory token balance is low");
		...
		setTransferedProperty(_property, msg.sender, transferdPropertyBalance.add(mulAmount));
	}

	function transferProperty(address nextPropertyDirectory) external onlyFactory {
	...
	function disassociate(address _recipient, address _property, uint _amount) external {
		...
		setTransferedProperty(_property, msg.sender, transferdPropertyBalance.sub(mulAmount));
	...
	function validatePropertyAddress(address _property) private view {
		address protocolConfig = IPropertyDirectoryConfig(configAddress()).getProtocolConfig();
