...
contract PropertyDirectoryFactory is UsingConfig {
	event Create(address indexed _propertyDirectory, address _sender, string _name, string _symbol);
	...
	function create(string memory _name, string memory _symbol) external returns (address){
		...
		p.createStorage();
		PropertyDirectoryToken token = new PropertyDirectoryToken(msg.sender, _name, _symbol);
		address diredtoryAddress = address(p);
		IPropertyDirectoryConfig(configAddress()).setToken(diredtoryAddress, address(token));
	...
	function reCreate(address _directory) external returns (address){
		address token = IPropertyDirectoryConfig(configAddress()).getToken(_directory);
		...
		IPropertyDirectoryConfig(configAddress()).setToken(newDiredtoryAddress, token);
